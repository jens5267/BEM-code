...
#with open(airfoil+'/'+airfoil+'_'+str(int(Reynolds/1000))+'k.txt', 'r') as fp:    
for r in reynolds:
    for p in paths:
        with open(str(p) + '/'+ str(p) + '_' + str(r) + 'k.txt' , 'r') as fp:
            for i in range (16):
                next(fp)
            data = fp.read()
            Alpha   = []
...
# =============================================================================
# =============================================================================
            if max(Alpha) != max(ref):
                  Alpha.append(max(ref))
                  c_L.append(0)
                  c_D.append(0)
# =============================================================================
# =============================================================================
...
for r in reynolds:
    for p in paths:                    
        with open(str(p) + '/'+ str(p) + '_' + str(r) +'k_interpolated.txt','w') as f:
            for i in range(len(ref)):
